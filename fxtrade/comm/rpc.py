...
class RPC(object):
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.open_order_id:
                    order = self._fxtrade.exchange.get_order(trade.open_order_id, trade.pair)
                # calculate profit and send message to user
                try:
                    current_rate = self._fxtrade.exchange.get_ticker(trade.pair, False)['bid']
                ...
                results.append(dict(
                    trade_id=trade.id,
                    pair=trade.pair,
                    market_url=self._fxtrade.exchange.get_pair_detail_url(trade.pair),
    ...
    def _rpc_balance(self, fiat_display_currency: str) -> Dict:
        ...
        output = []
        total = 0.0
        for coin, balance in self._fxtrade.exchange.get_balances().items():
            ...
            if coin == 'BTC':
                ...
                try:
                    if coin == 'USDT':
                        rate = 1.0 / self._fxtrade.exchange.get_ticker('BTC/USDT', False)['bid']
                    else:
                        rate = self._fxtrade.exchange.get_ticker(coin + '/BTC', False)['bid']
        ...
        def _exec_forcesell(trade: Trade) -> None:
            ...
            if trade.open_order_id:
                ...
                if order and order['status'] == 'open' \
                        ...
                        and order['side'] == 'buy':
                    self._fxtrade.exchange.cancel_order(trade.open_order_id, trade.pair)
            ...
            current_rate = self._fxtrade.exchange.get_ticker(trade.pair, False)['bid']
            self._fxtrade.execute_sell(trade, current_rate, SellType.FORCE_SELL)
    ...
    def _rpc_forcebuy(self, pair: str, price: Optional[float]):
        ...
        if not self._fxtrade.config.get('forcebuy_enable', False):
        ...
        stake_currency = self._fxtrade.config.get('stake_currency')
        ...
        # gen stake amount
        stakeamount = self._fxtrade._get_trade_stake_amount(pair)

        # execute buy
        if self._fxtrade.execute_buy(pair, stakeamount, price):
            trade = Trade.query.filter(Trade.is_open.is_(True)).filter(Trade.pair.is_(pair)).first()
            return trade
        else:
            return None
